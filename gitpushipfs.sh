...
wrap=$(perl -S ipfswrap.pl $symb $qm | xyml qm)
echo wrap: $wrap
echo git: http://gitea.localhost:8080/ipns/$key/$symb
echo urn: urn:$nid:$symb

echo -n 'head: '
curl http://gitea.localhost:8080/ipns/$key/$symb/refs/heads/master
...
prev=QmekkpM5xEpbxGnbc9WXszVNwHDQgb2RgV7NismP3eQwMA
fi
