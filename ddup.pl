...
my $IPFS_PATH = (exists $ENV{IPFS_PATH}) ? $ENV{IPFS_PATH} : $ENV{HOME}.'/ipfs/.../DEDUP';
...
while (<>) {
  ...
  if ($size > $s50M) {
    print LOG "%s: %.3fM too big ! %s\n",$fname,$size/1024/1024,$file;
    next;
  }

  #next if $file =~ m,\.ipfs/,;
  my $fname = (&fname($file))[1];
  if ($ad) {
    printf "%s ... ($ad)\n",$fname;
  } else {
    printf "%s \r",$fname;
  ...
  if ($fname =~ '^CIQ') {
    $mhash = "\x01\x70\x19".&varint($len/8).$binshake;
  } else {
  ...
  if (-e $zfile) {
    ...
    if ($etime <= $zetime) { # link IPFS file to an early file
       ...
       if ($nlink < 4) { # if less than 4 inodes : just remove and link the file
         ...
         print LOG "$fname: ${nlink}x $file, (but $sk32.data older)\n";
         my $tfile = $file.'~';
         if (-e $file.'~') {
